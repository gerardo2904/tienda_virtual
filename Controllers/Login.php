<?php
    class Login extends Controllers{
        ...
        public function login(){
            $data['page_tag'] = "Login - Oftalmo Tijuana";
            ...
            if($_POST){
                if(empty($_POST['txtEmail']) || empty($_POST['txtPassword'])){
                    ...
                    if(empty($requestUser)){
                        $arrResponse = array('status' => false, 'msg' => 'Usuario o contraseña incorrecto.');
                    }else{
                        $arrData = $requestUser;
                        if($arrData['status'] == 1){
                            $_SESSION['idUser'] = $arrData['idpersona'];
                            $_SESSION['nEmp'] = $arrData['nombreempresa'];
                            if ($arrData['img']){
                                $_SESSION['iEmp'] = $arrData['img'];
                            }else
                            {
                                $_SESSION['iEmp'] = 'oftalmo1.jpg';
                            }
                            $_SESSION['login']  = true;
            ...
            if(empty($_POST['idUsuario']) || empty($_POST['txtEmail']) || empty($_POST['txtToken']) || empty($_POST['txtPassword']) || empty($_POST['txtPasswordConfirm'])){
                ...
                if($strPassword != $strPasswordConfirm){
                    ...
                    if(empty($arrResponseUser)){
                        ...
                        if($requestPass){
                            $arrResponse = array('status' => true, 'msg' => 'Contraseña actualizada exitosamente.');
                        }else {
                            $arrResponse = array('status' => false, 'msg' => 'No es posibles realizar el proceso, intente más tarde.');
                        }
                    }
                }
            }
